...
const NewAssetRequestOverview = ({ newAssetData, handleRelodenewData }) => {
  ...
  async function getShowAssetWithStatus() {
    const res = await axios.get(
      "https://api-dot-react-migration-project.el.r.appspot.com/api/show_asset_with_status"
    );
  ...
  const handleRejectStatus = async (row, status) => {
    try {
      await axios.put("https://api-dot-react-migration-project.el.r.appspot.com/api/assetrequest", {
        _id: row._id,
  ...
  const handleAssignedSubmit = async () => {
    try {
      ...
      await axios.put("https://api-dot-react-migration-project.el.r.appspot.com/api/update_sim", {
        id: assetsName,
        status: "Allocated",
      });
