...
const SimOverview = () => {
  ...
  useEffect(() => {
    ...
    axios.get("http://192.168.29.167:8080/api/get_all_users").then((res) => {
  ...
  function handleParticularSimData(simId) {
    axios.get(`http://192.168.29.167:8080/api/get_single_sim/${simId}`).then((res) => {
  ...
  function handleTransfer() {
    if (selectedUserTransfer != "") {
      ...
      axios.post("http://192.168.29.167:8080/api/add_sim_allocation", {
  ...
  function handleSimAllocation() {
    if (selectedUserTransfer !== "") {
      ...
      axios.put("http://192.168.29.167:8080/api/update_sim", {
        id: modalData.sim_id,
