...
const OrderHistory = () => {
  ...
  useEffect(() => {
    axios
      .get("https://node-dev-server.onrender.com/api/get_delivery_boy")
  ...
  const handlePlaceOrder = async (row) => {
    ...
    await axios.post("https://node-dev-server.onrender.com/api/add_orderreq", {
  ...
  const getRowDetail = (row) => {
    ...
    axios
      .get(
        `https://node-dev-server.onrender.com/api/get_single_productdata/${row.product_id}`
      )
      .then((res) => {
