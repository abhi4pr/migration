...
const OrderHistory = () => {
  ...
  useEffect(() => {
    axios
      .post("http://192.168.29.167:8080/api/get_delivery_boy", {
  ...
  const handlePlaceOrder = async (row) => {
    ...
    await axios.post("http://192.168.29.167:8080/api/add_orderreq", {
  ...
  const getRowDetail = (row) => {
    ...
    setSelectedRow(row);
    axios
      .get(`http://192.168.29.167:8080/api/get_single_productdata/${row.product_id}`)
      .then((res) => {
  ...
  const columns = [
    ...
    {
      name: "Product Image",
      selector: (row) => <img src={row.Product_image} style={{width:"90px",height:"90px"}} />,
      sortable: true,
    },
    {
      name: "Order Quantity",
