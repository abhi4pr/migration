...
const SalaryWFH = () => {
  ...
  function gettingSliderData() {
    axios
      .get("https://api-dot-react-migration-project.el.r.appspot.com/api/get_month_year_data")
      .then((res) => {
  ...
  const handleMonthYearData = async () => {
    try {
      const response = await axios.post(
        "https://api-dot-react-migration-project.el.r.appspot.com/api/get_salary_by_month_year",
        {
          month: month,
          year: Number(year),
        }
  ...
  function handleAllDepartmentSalary() {
    axios
      .post("https://api-dot-react-migration-project.el.r.appspot.com/api/save_all_depts_attendance", {
        month: month,
        year: year,
      })
      .then(() => {
  ...
  useEffect(() => {
    ...
    if (department || month || year !== "") {
      axios
        .get("https://api-dot-react-migration-project.el.r.appspot.com/api/get_total_salary")
        .then((res) => setCard2Data(res.data.data[0]));

      axios
        .post("https://api-dot-react-migration-project.el.r.appspot.com/api/left_employees", {
          dept_id: department,
          month: month,
          year: year,
        })
    ...
    axios
      .post("https://api-dot-react-migration-project.el.r.appspot.com/api/new_joiners", {
        dept_id: department,
        month: month,
        year: year,
      })
    ...
    axios
      .post("https://api-dot-react-migration-project.el.r.appspot.com/api/check_salary_status", {
        month: month,
        year: year,
        dept: department,
      })
  ...
  function gettingDepartmentSalaryExists() {
    axios
      .post("https://api-dot-react-migration-project.el.r.appspot.com/api/get_distinct_depts", {
        month: month,
        year: year,
      })
  ...
  const handleAttendance = async () => {
    try {
      await axios.post("https://api-dot-react-migration-project.el.r.appspot.com/api/add_attendance", {
        dept: department,
      ...
      await axios.put(
        "https://api-dot-react-migration-project.el.r.appspot.com/api/update_attendence_status",
        {
          month: month,
          year: Number(year),
          dept: department,
