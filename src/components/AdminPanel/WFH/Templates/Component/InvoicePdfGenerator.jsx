...
const InvoicePdfGenerator = ({ data, setIsPreviewModalOpen, handleSubmit }) => {
  ...
  const handleGeneratePDF = async (e, data) => {
    ...
    await axios.put(`https://node-dev-server.onrender.com/api/update_attendance`, {
      attendence_id: data.attendence_id,
      attendence_status_flow: "Pending for invoice verification",
    });

    await axios.post(`https://node-dev-server.onrender.com/api/add_finance`, {
      attendence_id: data?.attendence_id,
    });

    await axios.put(`https://node-dev-server.onrender.com/api/update_salary`, {
      attendence_id: data?.attendence_id,
      sendToFinance: 1,
    });
