...
const ExePageDetailes = ({
  ...
  const handleUpdate = async (params) => {
    ...
    const response = await axios.get(
      `http://34.93.135.33:8080/api/campaignphase/singlephase/${params.row.phase_id}`
    );
    setSinglePhase(response?.data?.data?.commitment);

    const assCommit = await axios.get(`http://34.93.135.33:8080/api/assignment/commit/single/${params.row.ass_id}`)
  ...
  const handleExecute=async (params)=>{
    console.log(params)
    const response=await axios.post('http://34.93.135.33:8080/api/assignment/status',{
  ...
  const updateSingleCommitment=async (params)=>{
    ...
    // console.log(payload)
    const response=await axios.put(`http://34.93.135.33:8080/api/assignment/commit/single/${params.comm_id}`,payload)
  ...
  const handleAssignedSubmit = async () => {
    const response = await axios.post('http://34.93.135.33:8080/api/assignment/commit', assignedData)
