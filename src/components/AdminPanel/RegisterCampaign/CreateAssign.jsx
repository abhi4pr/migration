    ...
    try {
      ...
      if (isPreAss?.data?.data?.length > 0) {
        const assignment = await axios.get(
          `https://api-dot-react-migration-project.el.r.appspot.com/api/assignment/phase/${id}`
        );
  ...
  const handleSubmitAssign = async () => {
    try {
      const createAssignment = await axios.post(
        `https://api-dot-react-migration-project.el.r.appspot.com/api/assignment/bulk`,
        { pages: payload }
      );
