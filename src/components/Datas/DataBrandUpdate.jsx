...
const DataBrandUpdate = () => {
  ...
  useEffect(() => {
    axios
      .get(`https://node-dev-server.onrender.com/api/get_single_data/${id}`)
      .then((res) => {
    ...
    // axios
    //   .get("https://node-dev-server.onrender.com/api/get_all_data_Sub_categories")
    //   .then((res) => {
  ...
  const getCombinedData = async () => {
    if (brand) {
      axios
        .get(`https://node-dev-server.onrender.com/api/get_data_based_data_name/${brand}`)
        .then((res) => {
          setLogos(res.data);
  ...
  const removeImage = async (_id) => {
    if (_id == id) {
      ...
      var data = await axios.delete(
        `https://node-dev-server.onrender.com/api/delete_data/${_id}`,
        null
      );
      if (data) {
        getCombinedData();
  ...
  const handleSubmit = async (e) => {
    ...
    await axios.put(`https://node-dev-server.onrender.com/api/edit_data_new`, {
      _id: id,
      data_name: brandName,
      remark: remark,
    ...
    try {
      for (let i = 0; i < details.length; i++) {
        ...
        await axios.post(
          "https://node-dev-server.onrender.com/api/add_data",
          formData,
          {
            headers: {
